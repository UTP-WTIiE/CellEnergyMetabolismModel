...
namespace CellEnergyMetabolismModel.IndividualMetabolisms.PPP.Structs.Constants.Simplified
{
    public struct Simplified_RateG6P_NADP__PGL_NADPH_Constants : IDoubleArray, IRate
    {
        ...
        public double CalculateRate(double g6p_, double pgl)
        {
            var g6p = g6p_ / 0.09 * 2.6e-3;

            var i = Vf * g6p / K_G6P - 0.01 * Vf * pgl / K_PGL;
            var m = 1 + g6p / K_G6P + pgl / K_PGL;
            var V1 = i / m;

            var r = V1 / ReferenceRate * ReferenceRateProbability;
            return r;
        }
        ...
        public PPPCellState Queue(IPPPCellState currentState, double delta, Func<double> univariate_random_fn, bool modify_start_products = true, bool modify_nadph_level = false)
        {
