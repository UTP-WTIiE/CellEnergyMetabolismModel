...
namespace CellEnergyMetabolismModel.IndividualMetabolisms.PPP.Structs.Constants
{
    public struct RatePGL_H20__6PG_H_Constants : IDoubleArray, IRate
    {
        ...
        public double CalculateRate(double pgl_, double _6pg)
        {
            var pgl = pgl_ / 0.015 * 5e-6;
            var V4a = (Vf * (pgl / K_PGL) - (Vr * (_6pg / K_6PG))) / (1 + (pgl / K_PGL) + (_6pg / K_6PG));

            return V4a / ReferenceRate * ReferenceRateProbability * 10;
        }
